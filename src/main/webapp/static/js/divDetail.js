...
function showDivDetail(url,width,height,top,left,depiction){
	...
	//指令审批专用
	var bizPopType = false;
	if(depiction == 'bizPop') {
		depiction = "<font color='red'>指令审批消息提示框</font>";
		bizPopType = true;
	}
	...
	if(bizPopType) {
		strHTML += "    <td align=right ><a href='javascript:closeMyPop(true)'>关闭</a></td>";
	} else {
...
function closeMyPop(){
	...
	//指令审批专用
	if(arguments[0]==true) {
		var iframe = document.frames["winPopTip"];
		iframe.commit_(5);
	}
}
var dragX=0;
