...
var topY=0,leftX=0;  //自定义定位偏移量 2007-02-11  Liang添加  
//选择日期 →  Liang 2007-06-10 添加，通过 ID 来选日期  
...
//选择日期 →  Liang 2006-06-25 添加  
function SelectDate(obj,strFormat,x,y)  
{  
 
    leftX =(x == null) ? leftX : x;  
    topY  =(y == null) ? topY : y;//自定义定位偏移量 2007-02-11  Liang添加   
 ...
 * @param p the pattern of your date 
 2006-06-25  Liang 修改为根据用户指定的 style 来确定； 
 */ 
...
function Calendar(beginYear, endYear, lang, dateFormatStyle) {  
  ...
  "sel_bg"        : "#FFCCCC",  //已被选择的日期单元格背影色 2006-12-03 Liang添加    
...
/* //2008-01-29 放到了 show ，因为要做 pickMode 判断 
  ...
  */ 
  //this.changeSelect();  
  //this.bindData(); //2006-12-30 民工.砖家注释  
}  
...
Calendar.prototype.draw = function() {  
  ...
  var mvAry = [];  
  //mvAry[mvAry.length]  = '  <form name="calendarForm" style="margin: 0px;">'; //因 <form> 不能嵌套， 2006-12-01 Liang改用 Div  
...
//2009-03-03 添加的代码，放置时间的行  
  ...
  { calendar.ReturnDate(""); /*calendar.dateControl.value = "";calendar.hide();*///2007-09-14 Liang注释  
  }  
...
Calendar.prototype.bindYear = function() {  
  var cy = this.calendarYear;//2006-12-01 Liang修改  
  cy.length = 0;  
...
Calendar.prototype.bindMonth = function() {  
  var cm = this.calendarMonth;//2006-12-01 Liang修改  
  cm.length = 0;  
  for (var i = 0; i < 12; i++){  
...
Calendar.prototype.update = function (e){  
  this.year  = e.calendarYear.options[e.calendarYear.selectedIndex].value;//2006-12-01 Liang修改  
...
Calendar.prototype.bindData = function () {  
 ...
 // var dateArray = this.getMonthViewArray(this.date.getYear(), this.date.getMonth());  
  //2006-12-30 民工.砖家修改 在Firefox 下年份错误  
 ...
 var tds = this.getElementById("calendarTable").getElementsByTagName("td");  
  ...
  tds[i].style.backgroundColor = calendar.colors["td_bg_out"];  
    ...
    {  
      ...
      tds[i].style.cursor ="pointer"; //2007-08-06 Liang添加，鼠标变成手指状  
...
Calendar.prototype.show = function (dateObj, popControl) {  
  ...
  var now =  new Date();  
  this.date = (dateObj.value.length > 0) ? new Date(dateObj.value.toDate(this.dateFormatStyle)) : now.format(this.dateFormatStyle).toDate(this.dateFormatStyle) ;//2008-01-29 Liang添加 → 若为空则根据dateFormatStyle初始化日期  
  ...
  this.panel.style.left = (xy.x + leftX)+ "px"; //Liang 2007-02-11 修改 → 加入自定义偏移量  
  this.panel.style.top = (xy.y + topY + dateObj.offsetHeight) + "px";  
    
  //Liang 2006-06-25 修改 → 把 visibility 变为 display，并添加失去焦点的事件  //this.setDisplayStyle("select", "hidden");  
...
//焦点转移时隐藏日历 → Liang 2006-06-25 添加  
Calendar.prototype.onblur = function() {  
    if(!isFocus){this.hide();}  
}  
 
//以下Liang 2007-07-26 修改 → 确保日历容器节点在 body 最后，否则 FireFox 中不能出现在最上方  
